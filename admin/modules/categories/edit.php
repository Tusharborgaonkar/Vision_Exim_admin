...
$id = (int)($_GET['id'] ?? 0);
if ($id <= 0) {
    header('Location: list.php?error=' . urlencode('Invalid category ID.'));
    exit;
}
...
if (!$category) {
    header('Location: list.php?error=' . urlencode('Category not found.'));
    exit;
}
...
// Fetch potential parent categories (excluding this category itself to prevent recursive parent loop!)
$parent_cats = [];
$res = $conn->prepare("SELECT id, name FROM categories WHERE parent_id IS NULL AND status = 'active' AND id != ? ORDER BY name ASC");
$res->bind_param('i', $id);
$res->execute();
$parent_res = $res->get_result();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Handle Image Removal via direct post request
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("UPDATE categories SET name = ?, slug = ?, parent_id = ?, description = ?, status = ?, sort_order = ?, image = ? WHERE id = ?");
        $stmt->bind_param('ssissisi', $name, $slug, $parent_val, $description, $status, $sort_order, $image_path, $id);
        
        if ($stmt->execute()) {
            header('Location: list.php?success=' . urlencode('Category "' . $name . '" updated successfully!'));
            exit;
        } else {
...
<main class="lg:ml-[270px] pt-[70px] min-h-screen sidebar-transition">
    <div class="p-6 lg:p-8 page-content">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div>
                <div class="flex items-center gap-2 text-[12px] text-gray-400 dark:text-slate-500 mb-1">
                    ...
                    <span>/</span>
                    <span class="text-gray-600 dark:text-slate-400 font-medium">Edit Category</span>
                </div>
                <h1 class="text-[22px] font-bold text-spice-dark dark:text-white">Edit Category: <?= htmlspecialchars($category['name']) ?></h1>
                <p class="text-[13px] text-gray-400 dark:text-slate-500 mt-1">Modify classification group properties</p>
            </div>
            <div>
        ...
        <!-- Form Form -->
        <form method="POST" action="" enctype="multipart/form-data" id="editForm" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-card border border-gray-100/50 dark:border-slate-700 p-6">
                    ...
                    <div class="grid grid-cols-1 gap-5">
                        <div>
                            ...
                            <input type="text" id="categoryName" name="name" value="<?= htmlspecialchars($old_name) ?>" required
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="categorySlug" name="slug" value="<?= htmlspecialchars($old_slug) ?>" readonly
                        ...
                        <div>
                            ...
                            <select name="parent_id" class="w-full px-4 py-3 rounded-xl border-2 border-gray-100 dark:border-slate-700 bg-gray-50/50 dark:bg-slate-700/50 text-[12px] text-gray-600 dark:text-slate-300 outline-none focus:border-spice-green-600 cursor-pointer">
                                <option value="none" <?= empty($old_parent) ? 'selected' : '' ?>>None (Parent Category)</option>
                        ...
                        <div>
                            ...
                            <textarea rows="4" name="description" placeholder="Description"
                                      class="w-full px-4 py-3 rounded-xl border-2 border-gray-100 dark:border-slate-700 bg-gray-50/50 dark:bg-slate-700/50 text-[12px] text-spice-dark dark:text-slate-200 focus:border-spice-green-600 focus:bg-white dark:focus:bg-slate-700 focus:ring-4 focus:ring-spice-green-600/10 outline-none transition-all"><?= htmlspecialchars($old_desc) ?></textarea>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <div class="relative rounded-xl overflow-hidden group border border-gray-100 dark:border-slate-700 mb-3" id="imagePreviewContainer">
                                ...
                                <img id="imgPreview" class="w-full h-32 object-cover hidden">
                                <?php endif; ?>
                                ...
                                <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1.5">
                                    <label class="w-7 h-7 rounded-lg bg-white text-gray-700 hover:bg-gray-100 flex items-center justify-center shadow cursor-pointer" title="Upload New">
                                    ...
                                    <button type="button" onclick="removeCurrentImage()" class="w-7 h-7 rounded-lg bg-spice-chili-500 text-white hover:bg-spice-chili-600 flex items-center justify-center shadow" title="Remove">
...
function removeCurrentImage() {
    ...
    // Show fallback or clear container visual
    ...
    if (container) {
        ...
        if (!fallback) {
            ...
            fallback.className = 'w-full h-28 bg-spice-turmeric-100 dark:bg-slate-700 flex items-center justify-center';
            fallback.innerHTML = '<i class="fas fa-pepper-hot text-4xl text-spice-turmeric-500"></i>';
